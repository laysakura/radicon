...
/*==============================================================================
*   MODULE        : lcd_put_sho
*   FUNCTION      : Data をshoくんのいうように000(STOP),001(GO),010(BACK),011(RIGHT),100(LEFT) LCD へ表示
*   ARGUMENT      : data : shoくんの指示する000,001,010,011,100
*   RETURN        : none
*   NOTE          : none
*===============================================================================*/
void lcd_put_sho(unsigned char data)
{
    unsigned char stop[4] = {0x53, 0x54, 0x4F, 0x50};          /* "STOP"        */
    unsigned char go[2] = {0x47, 0x4F}     /* "GO"    */
    unsigned char back[4] = {0x42, 0x41, 0x43, 0x4B};     /* "BACK"   */
	unsigned char right[5] = {0x52, 0x49, 0x47, 0x48, 0x54};          /* "RIGHT"        */
    unsigned char left[4] = {0x4C, 0x45, 0x46, 0x54};     /* "LEFT"    */
	...
	case 0x01:
		...
		break;
	case 0x02:
		for(i=0;i<CHAR_BACK;+i){
			lcd_put_char(back[i]);
		}
		break;
	case 0x03:
		for(i=0;i<CHAR_RIGHT;+i){
			lcd_put_char(right[i]);
		}
		break;
	case 0x04:
		for(i=0;i<CHAR_LEFT;+i){
			lcd_put_char(left[i]);
		}
		break;
	};	//switchでdataの場合わけをおこなってlcd_put_charを使う。
