...
/*  プロトタイプ宣言                                                            */
/*==============================================================================*/
unsigned char key_input_sho(void);            /* Key Check 処理               */
...
////////////////////////////////////////////////////////////////////////////////////改造！！もっと変える
/*==============================================================================
*   MODULE        : key_input_sho
*   FUNCTION      : shoのＰＣからの入力をキーマトリクスにて検出(KEY Output:4PIN)
*   ARGUMENT      : none
*   RETURN        : code : キー押下状態（押下されていれば Key_Codeを返す）
*   NOTE          : Key Output Pin を Hi にして Key Input Pin をチェック
*===============================================================================*/
unsigned char key_input_sho(void)
{
	...
	//今回の試作では、INPUTの種類が５個と少ないため、KEY_INPUT_NUM(4)ではなくKEY_INPUT_NUM_SHO(2)を使う。
    for (i=0; i<KEY_INPUT_NUM_SHO; i++) {
        switch (i) {
          case 0:
...
/////////////////////////////////////////////////////////////////////////////////////////こいつを改造する。
    ...
    if ((PORTA & MODE_MASK) == RUN_MODE_1) {
        ...
        {
            /* Key Check */
            key_code = key_input_sho();///////////////////////key_input_checkをshoくんの信号を受け取れるように改良（上）
            ...
            if (key_code == KEY_OFF) {
                ...
                if (key_code <= KEY_CODE04) {
                	...
                	/* lcd.cで定義したlcd_pu_sho(unsigned char data)を使ってＬＣＤに表示      */
                	lcd_put_sho(func_key_type);
                	/* 対応したコードを送信 */
                    ir_out_start(func_key_type, key_code);///////そんまま流用笑///////////あとは回路上でshoの信号の渡し方を考える。
                }
            }
        }
    }
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if ((PORTA & MODE_MASK) == RUN_MODE_4) {
        ...
        {
            /* IR受信 */
            ir_state = ir_recieve();/////////////////////////////////////////////////////////////////////////ir_stateではなく、ＰＣからの出力にする。
